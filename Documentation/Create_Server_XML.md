...
```game_mode_cfg``` - полное название CFG-файла режима (например, если карта у вас для мясорубки, то вы пишите ```game_mode_cfg="ffa_mode.cfg"```, данные файлы хранятся по пути: \[\config\content\\];

```uid``` - уникальный идентификатор карты (уникальный идентификатор - это 128-битное целое число, в котором могут использоваться: строчные латинские буквы, цифры и дефисы в качестве разделителей, общий вид: ```********-****-****-****-************```). Можно сгенерировать на сайте: [Free Online GUID Generator](https://www.guidgenerator.com/);
...
```difficulty_cfg``` - полное название CFG-файла сложности миссии, данные файлы хранятся по пути: \[\config\content\\];
